...
export class LiqPool {
    ...
    treasury_cut: Fee;
    lp_supply: BN;
    lent_from_sol_leg: BN;
    liquidity_sol_cap: BN;

    constructor({lp_mint, lp_mint_authority_bump_seed, sol_leg_bump_seed, st_sol_leg_authority_bump_seed, st_sol_leg, lp_liquidity_target, lp_max_fee, lp_min_fee, treasury_cut, lp_supply, lent_from_sol_leg, liquidity_sol_cap, }: {lp_mint: Pubkey, lp_mint_authority_bump_seed: number, sol_leg_bump_seed: number, st_sol_leg_authority_bump_seed: number, st_sol_leg: Pubkey, lp_liquidity_target: BN, lp_max_fee: Fee, lp_min_fee: Fee, treasury_cut: Fee, lp_supply: BN, lent_from_sol_leg: BN, liquidity_sol_cap: BN, }) {
        ...
        this.lent_from_sol_leg = lent_from_sol_leg;
...
export class StakeSystem {
    ...
    delayed_unstake_cooling_down: BN;
    ...
    constructor({stake_list, delayed_unstake_cooling_down, stake_deposit_bump_seed, stake_withdraw_bump_seed, slots_for_stake_delta, last_stake_delta_epoch, min_stake, extra_stake_delta_runs, }: {stake_list: List_StakeDiscriminator_StakeRecord_u32_, delayed_unstake_cooling_down: BN, stake_deposit_bump_seed: number, stake_withdraw_bump_seed: number, slots_for_stake_delta: BN, last_stake_delta_epoch: BN, min_stake: BN, extra_stake_delta_runs: number, }) {
        this.stake_list = stake_list;
        this.delayed_unstake_cooling_down = delayed_unstake_cooling_down;
...
export class State {
    ...
    lent_from_reserve: BN;
    min_deposit: BN;
    min_withdraw: BN;
    staking_sol_cap: BN;
    emergency_cooling_down: BN;

    constructor({st_sol_mint, admin_authority, operational_sol_account, treasury_msol_account, reserve_bump_seed, st_mint_authority_bump_seed, rent_exempt_for_token_acc, reward_fee, stake_system, validator_system, liq_pool, available_reserve_balance, st_sol_supply, st_sol_price, circulating_ticket_count, circulating_ticket_balance, lent_from_reserve, min_deposit, min_withdraw, staking_sol_cap, emergency_cooling_down, }: {st_sol_mint: Pubkey, admin_authority: Pubkey, operational_sol_account: Pubkey, treasury_msol_account: Pubkey, reserve_bump_seed: number, st_mint_authority_bump_seed: number, rent_exempt_for_token_acc: BN, reward_fee: Fee, stake_system: StakeSystem, validator_system: ValidatorSystem, liq_pool: LiqPool, available_reserve_balance: BN, st_sol_supply: BN, st_sol_price: BN, circulating_ticket_count: BN, circulating_ticket_balance: BN, lent_from_reserve: BN, min_deposit: BN, min_withdraw: BN, staking_sol_cap: BN, emergency_cooling_down: BN, }) {
