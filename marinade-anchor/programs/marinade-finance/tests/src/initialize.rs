...
use marinade_finance_offchain_sdk::{
    ...
    transaction_builder::{PreparedTransaction, TransactionBuilder},
    marinade_finance::{Fee, State, MAX_REWARD_FEE, liq_pool::LiqPool,}
};
use marinade_finance_offchain_sdk::anchor_lang::solana_program::{clock::Epoch, native_token::LAMPORTS_PER_SOL};
...
lazy_static! {
    static ref CREATOR_AUTHORITY: Arc<Keypair> =
        Arc::new(Keypair::from_bytes(&include!("../../../../keys/creator.json")).unwrap());
}
