...
// Epochs
handlers.epochs = function (data, callback) {
  var acceptableMethods = ["post", "get", "put", "delete"];
  if (acceptableMethods.indexOf(data.method) > -1) {
    handlers._epochs[data.method](data, callback);
  } else {
    callback(405);
  }
};
...
handlers._epochs.post = function (data, callback) {
  var epochNum =
  ...
  /**
   * Ignore neglible validators (neglible: stake_status == "None", pct == '0.0')
   * Order buy recent epoch and for each epoch order by active_stake
   */
  let sql =
    "SELECT * FROM scores as s WHERE epoch > -1 AND stake_state != 'None' AND pct != '0.0'  ORDER BY epoch DESC, active_stake DESC";
  let params = [];
  var holder = [];
...
function generateNewValidators() {
  //Dependencies
  const sqlite3 = require("sqlite3").verbose();
  const fs = require("fs");
  // open database from file
...
handlers._epochs.get = function (data, callback) {
  // generateNewValidators();
...
handlers._epochs.put = function (data, callback) {
  console.log("placed");
  callback(200);
};
handlers._epochs.delete = function (data, callback) {
  console.log("deleted");
  callback(200);
};
