...
handlers._validators.post = function (data, callback) {
  ...
  //query
  let sql =
    "SELECT * FROM scores2 as s WHERE epoch > -1 AND marinade_staked > 0 ORDER BY epoch DESC, rank ASC";
  let params = [];
  var holder = [];
    ...
    if(data.nestedRoute[1] == 'count'){
      _data.read("epochs", "validators", function (err, data) {
        if (!err && data) {
          ...
          var mostRecentEpoch = entries[0].epoch_stats[0].epoch
          // console.log(mostRecentEpoch);
          
          var count = 0;
          var index = 0;
          // Only search validator data of most recent epoch
          while(entries[index].epoch_stats[0].epoch == mostRecentEpoch){
            if(entries[index].epoch_stats[0].marinade_staked > 0){
              count++;
            }
            index++;
          }
