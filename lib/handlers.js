...
handlers._epochs.post = function (data, callback) {
  ...
  //query
  let sql =
    "SELECT * FROM scores as s WHERE epoch > -1 ORDER BY identity, epoch";
  let params = [];
  var holder = [];
...
handlers._epochs.get = function (data, callback) {
  ...
  _data.read("epochs", "validators", function (err, data) {
    if (!err && data) {
      ...
      // Pare data into array
      const entries = Object.entries(data);

      // Populate result object
      result.totalPages = Math.ceil(entries.length / limit);
      result.validators = entries.slice(startIndex, endIndex);

      if(result.totalPages < page || page < 1) { callback(404); return; } // 404 if page is negative or over last page
      callback(200, result);
    } else {
      callback(404);
    }
