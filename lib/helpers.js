...
helpers.generateValidators = function() {
  ...
  /**
   * 
   * SELECT scores.vote_address,scores.name,  scores.epoch, scores.pct, scores2.rank, scores2.score, scores2.credits_observed, scores2.commission, scores2.apy, scores2.marinade_staked, scores.avg_position as average_active_stake
FROM scores
LEFT JOIN scores2 ON  scores.epoch = scores2.epoch AND scores2.vote_address = scores.vote_address
UNION ALL
SELECT scores2.vote_address, scores2.name, scores2.epoch, scores2.pct, scores2.rank, scores2.score, scores2.credits_observed, scores2.commission, scores2.apy, scores2.marinade_staked, scores2.avg_active_stake as average_active_stake
FROM scores2
WHERE scores2.marinade_staked > 0 
ORDER BY epoch DESC , scores2.rank ASC
...
ORDER BY epoch DESC , scores2.rank ASC
   */
  let sql =
    "SELECT scores.epoch as epoch, scores.name,  scores2.rank as rank, scores.pct, scores2.score, scores2.commission, scores2.avg_active_stake as average_active_stake, scores2.apy, scores2.delinquent, scores2.marinade_staked, scores2.should_have, scores2.data_center_concentration \
    FROM scores2 \
    INNER JOIN scores ON scores.vote_address = scores2.vote_address WHERE scores2.marinade_staked > 0 \
    UNION ALL \
    SELECT scores2.epoch as epoch, scores2.name, scores2.rank as rank, scores2.pct, scores2.score, scores2.commission, scores2.avg_active_stake as average_active_stake, scores2.apy, scores2.delinquent, scores2.marinade_staked, scores2.should_have, scores2.data_center_concentration \
    FROM scores2 \
    WHERE scores2.marinade_staked > 0 \
